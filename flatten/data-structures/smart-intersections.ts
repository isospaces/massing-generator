/*
    ...
    This function are supposed for intEdteernal usage by morphing and relation methods between
 */
import { VertexType } from "../utils/constants";
import { Vector } from "../classes/vector";
import { Edge } from "../classes/edge";
import { EQ } from "../utils/utils";

export const addToIntPoints = (edge: Edge, point: Vector, intersections: Vector[]) => {
  let id = intersections.length;
  ...
  let is_vertex = VertexType.NOT_VERTEX;
  if (EQ(len, 0)) {
    is_vertex |= VertexType.START_VERTEX;
  }
  if (EQ(len, edge.shape.length)) {
    is_vertex |= VertexType.END_VERTEX;
  }
  // Fix intersection point which is end point of the last edge
  let arcLength = is_vertex & VertexType.END_VERTEX && edge.next?.arcLength === 0 ? 0 : edge.arcLength + len;
...
export function getSortedArray(intersections: Vector[]) {
  let faceMap = new Map();
  let id = 0;
  // Create integer id's for faces
  for (let point of intersections) {
    if (!faceMap.has(point.face)) {
      faceMap.set(point.face, id);
      id++;
    }
  }
  ...
  // Clone and sort
  let int_points_sorted = intersections.slice().sort(compareFn);
...
export function filterDuplicatedIntersections(intersections) {
  ...
  for (let i = 1; i < intersections.int_points2_sorted.length; i++) {
    ...
    if (
      int_point_ref2.id == -1 /* can't be reference if already deleted */ ||
  ...
  if (do_squeeze) {
    intersections.int_points1 = intersections.int_points1.filter((intersection) => intersection.id >= 0);
    intersections.int_points2 = intersections.int_points2.filter((intersection) => intersection.id >= 0);
...
export function initializeInclusionFlags(int_points) {
  ...
  for (let int_point of int_points) {
    int_point.edge_before.bvEnd = VertexType.BOUNDARY;
    int_point.edge_after.bvStart = VertexType.BOUNDARY;
  }
}
...
export function setOverlappingFlags(intersections) {
  ...
  for (let i = 0; i < num_int_points; i++) {
    ...
    if (!(edge_from1.bv === VertexType.BOUNDARY && edge_to1.bv === VertexType.BOUNDARY))
    ...
    if (!(edge_from2.bv === VertexType.BOUNDARY && edge_to2.bv === VertexType.BOUNDARY && edge_from2 === edge_to2)) {
...
export function intPointsPoolCount(int_points, cur_int_point_num, cur_face) {
  ...
  for (let i = cur_int_point_num + 1; i < int_points.length; i++) {
    ...
    if (
      !(
        int_point_next.pt.equalTo(int_point_current.pt) &&
...
export function splitByIntersections(polygon, int_points) {
  ...
  for (let int_point of int_points) {
    ...
    int_point.is_vertex = VertexType.NOT_VERTEX;
    if (edge.shape.start && edge.shape.start.equalTo(int_point.pt)) {
      int_point.is_vertex |= VertexType.START_VERTEX;
    }
    if (edge.shape.end && edge.shape.end.equalTo(int_point.pt)) {
      int_point.is_vertex |= VertexType.END_VERTEX;
    }

    if (int_point.is_vertex & VertexType.START_VERTEX) {
      // nothing to split
      int_point.edge_before = edge.prev;
      int_point.is_vertex = VertexType.END_VERTEX;
      continue;
    }
    if (int_point.is_vertex & VertexType.END_VERTEX) {
      // nothing to split
      continue;
    }
...
export function insertBetweenIntPoints(int_point1, int_point2, newEdge: Edge) {
  let edge_before = int_point1.edge_before;
  let edge_after = int_point2.edge_after;

  edge_before.next = newEdge;
  newEdge.prev = edge_before;

  newEdge.next = edge_after;
  edge_after.prev = newEdge;
}
